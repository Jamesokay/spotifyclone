import React from 'react'

export default function PlaylistLoader() {
    return (
        <div>
        <table className='skeletonTable' cellSpacing='0' cellPadding='0'>
          <thead>
            <tr>
            <th className='empty'></th>
            <th style={{textAlign: 'center'}}>#</th>
            <th>TITLE</th>
            <th></th>
            <th>ALBUM</th>
            <th style={{width: '4%'}}></th>
            <th style={{width: '2.5%'}}>
            <svg className='timeIcon' style={{float: 'right'}} width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path className='clock' d="M7.999 3H6.999V7V8H7.999H9.999V7H7.999V3ZM7.5 0C3.358 0 0 3.358 0 7.5C0 11.642 3.358 15 7.5 15C11.642 15 15 11.642 15 7.5C15 3.358 11.642 0 7.5 0ZM7.5 14C3.916 14 1 11.084 1 7.5C1 3.916 3.916 1 7.5 1C11.084 1 14 3.916 14 7.5C14 11.084 11.084 14 7.5 14Z" fill="currentColor"></path>
            </svg>
            </th>
            <th style={{width: '5%'}}></th>
            <th className='empty'></th>
            </tr>
          </thead>

          <tbody>
          <tr>
            <td style={{height: '10px'}}></td>
          </tr>
          <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'></td>
              <td className='emptyCell'></td>
            </tr>
            <tr className='trackTableRow'>
            <td className='emptyCell'></td>
              <td className='rowFirst tdRegTrack'><div className='skeletonIndex'/></td>        
              <td className='tableImgCol tdRegTrack'><div className='skeletonImg'/></td>
              <td className='tdRegTrack'>
                <div className='skeletonTitle'></div> 
                <div className='skeletonArtist'></div>
              </td>
              <td className='tdRegTrack' style={{width: '505px'}}>
                <div className='skeletonAlbum'></div>
              </td>
              <td></td>
              <td className='tdRegTrack'><div className='skeletonTime'/></td>
              <td className='tdRegTrack rowLast'>
              </td>
              <td className='emptyCell'></td>
            </tr>
          </tbody>
        </table>
      </div>
    )
}
